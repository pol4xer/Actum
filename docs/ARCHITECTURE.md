...
Simulator и реального ожидания. После достижения плановой отметки timer остаётся в
open-ended work-состоянии без deadline и считает фактическое время до ручного завершения;
абсолютный `startedAt` сохраняет корректный overtime после сворачивания приложения.
...
Research cache принадлежит всей программе, а не одному циклу. Стабильный
`researchAnchor` учитывает цель, исходные level/baseline и дневной лимит, но не retry cap.
Версионный cache key дополнительно включает research prompt/parser/model, а бриф хранится 400 дней.
Plan cache остаётся 30-минутным и включает baseline, cycleNumber и programContext. Новый месяц
требует явного planning-запроса, но не повторяет web-search; если совместимый research потерян,
gateway останавливается до нового provider POST.
...
2. Сохранённые plan-v1–plan-v6 продолжают открываться через совместимый runner и миграцию v3.
3. `plan-v7` трактует 1/6/12 месяцев как предел продолжения, содержит самый ранний `targetCycleNumber`, roadmap и ровно один 30-дневный цикл; каждый день имеет основной измеримый timer/counter.
4. Изменение prompt/contract/validator/parser/model меняет соответствующий cache key.
5. `researchAnchor` обезличенно закрепляет research за одной программой; текущие версии research-промпта, parser и модели всё равно входят в cache key.
6. `reuseOnly` никогда не открывает путь создания нового OpenAI response.
7. Route-файлы кроме `_layout.tsx` остаются тонкими.
8. Межмодульный импорт идёт через публичный `index.ts`.
9. Серверная JSON Schema и клиентская Zod-схема plan-v7 структурно совпадают для всех
   дневных лимитов, пределов продолжения и номеров циклов.
10. DEV-функции включаются только через `src/config` при `__DEV__` и точном
   `EXPO_PUBLIC_ACTUM_MODE=development`; неизвестные значения считаются production.
11. Завершение 30 дней закрывает только текущий цикл. Большая числовая цель завершается
    фактическим результатом успешного primary timer/counter, достигшим target; если это не
    произошло раньше, используется assessment цикла. Новый платный planning-запрос возможен
