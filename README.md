...
- фиксированный срок достижения: один месяц, полгода или год — без произвольного текстового срока;
...
- GPT-план из трёх фаз: исходная и конечная метрика, расчёты нагрузки, упражнения, подходы, точный объём, темп, отдых и проверяемый критерий;
...
[actum-ai] prompt=actum-plan-2026-09-04-adaptive-program-v1 research_prompt=actum-research-2026-09-04-program-v3 contract=plan-v6 validator=plan-validator-v6 baseline_parser=numeric-metric-v2 transport=background-polling durable_state=ready
```
...
- завершённый program-level web-research сохраняется на 400 дней, поэтому ошибки planning, повторы и следующие месячные циклы используют тот же бриф без нового поиска (`research_cache_hit`);
...
Если приложение перезапустилось до принятия плана и React-форма очистилась, экран новой цели читает локальный `GET /saved-plan/latest`, повторно проверяет последний completed plan-v6 response и открывает его ревью автоматически. Этот маршрут read-only: он не вызывает OpenAI и восстанавливает исходный input snapshot. Старые provider-ответы plan-v5 остаются на диске, но не маскируются под новый контракт.

Сохранённые локальные plan-v1–plan-v5 мигрируют без удаления в schema v3 и продолжают открываться через совместимый runner. Для старой годовой цели Actum отделит первый 30-дневный цикл от всей программы и больше не отметит цель завершённой только из-за окончания этого цикла. Чтобы получить исследованную roadmap и контрольный замер plan-v6, нужна новая явная генерация.
...
```text
цель + измеренная исходная точка + фиксированный срок (месяц / полгода / год) + время в день
...
→ строгий JSON plan-v6: конечная метрика + roadmap на 1/6/12 циклов + текущий цикл
→ локальная проверка конечной цели, roadmap, контрольного замера и дневного лимита
...
→ День 30 измеряет результат внутри приложения
...
`plan-v6` не принимает общую фразу вместо тренировки и не хранит срок как свободный текст. Пользователь вводит измеренную исходную точку — например, «обычно 40 секунд; рекорд 1:20» — и выбирает месяц, полгода или год. Локальный parser до платного планирования отдельно нормализует baseline и числовую конечную цель: `1:20 → 80 seconds`, `10 минут → 600 seconds`. GPT не может заменить эти числа собственными. Если значение неоднозначно, parser не угадывает его.

Модель обязана вернуть roadmap на весь выбранный срок и ровно 30 самостоятельных объектов `days` текущего цикла с номерами `1…30`; `repeatCount`, «повтори вчера», скрытое клонирование и локальная перенумерация перепутанных дней не используются. Числовая roadmap монотонно ведёт от совместимого baseline к конечной метрике, а последний ориентир обязан совпадать с целью. Каждый день содержит `execution.kind = in_app` и один или несколько блоков:
...
День 30 содержит явный assessment-блок timer/counter. Actum сохраняет фактическое значение из `MissionRun`, показывает его рядом с конечной целью и не завершает многомесячную цель после первого цикла. Кнопка «Собрать месяц N» отправляет новый planning-запрос только после подтверждения пользователя; сохранённый program-level web-research переиспользуется, завершённые этапы roadmap не переписываются, а подробные нагрузки следующих 30 дней адаптируются к новому baseline.

Для каждой новой цели `Web research` обязателен: модель сначала выполняет не меньше трёх различающихся поисков и анализирует источники, а затем отдельным запросом строит JSON-план. Исследовательский этап считается успешным только при наличии минимум двух реальных URL citation. Внутренний режим без нового поиска оставлен только для продолжения старых plan-v1–plan-v5, чей прежний research нельзя надёжно связать с новым контрактом; отдельного переключателя в интерфейсе нет. OpenAI-этапы запускаются через background mode и опрашиваются по response ID, поэтому многоминутная работа не держится на одном хрупком HTTP-соединении. Даже terminal/refusal с известным response ID остаётся привязан к этому ID: повтор делает GET того же результата, а не новый POST.
...
`pnpm test:ai` использует фальшивый OpenAI, не открывает сетевые сокеты и не тратит API-деньги. Он воспроизводит сбои POST и polling, проверяет guard/response ID/кэши, reuse-only, resume, `inflight_join`, 400-дневное переиспользование research, связанное восстановление источников и блокировку неразрешимого assessment до платного запроса. Также проверяются строгий plan-v6, конечная метрика, roadmap 1/6/12 циклов, неизменность завершённых этапов, адаптация следующего baseline, возрастающие и уменьшающиеся цели, миграция v1/v2→v3, MissionRun, подготовка `3 → 2 → 1`, DEV-пропуск дней, совместимость plan-v1–plan-v5, DTO/server parity и архитектурные границы. `pnpm check` запускает TypeScript, все локальные тесты и production export web-версии.
