...
[actum-ai] prompt=actum-plan-2026-07-31 contract=plan-v2
```
...
При каждом реальном запросе сервер печатает этапы `received → researching → planning → completed`, локальный request ID, OpenAI response ID, количество web-поисков, токены и время. Если этих строк нет, запрос до OpenAI не дошёл. Usage нужно смотреть в том OpenAI API Project, которому принадлежит `sk-proj-...` ключ; это не история и не подписка ChatGPT.
...
Режим `Web research` включён по умолчанию: модель сначала ищет и анализирует источники, а затем отдельным запросом строит JSON-план. `Быстрый GPT` пропускает поиск. Для длинных исследований уровня многостраничного отчёта позже понадобится background polling; текущий MVP ограничивает исследование задачей построения практического маршрута.
