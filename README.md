...
- сворачиваемый план по дням, журнал сессий и сравнение «сейчас vs по плану»;
...
[actum-ai] prompt=actum-plan-2026-09-04-action-first-v6 research_prompt=actum-research-2026-08-01-closed-loop-v2 contract=plan-v5 validator=plan-validator-v5 baseline_parser=baseline-v1 transport=background-polling durable_state=ready
```
...
Сохранённые plan-v1–plan-v4 остаются читаемыми и завершаются через старый runner. Чтобы проверить новый закрытый plan-v5, открой «Настройки» → «Новая цель»: профиль и XP останутся, текущий маршрут и его журнал будут очищены после подтверждения.
...
→ полная сессия остаётся в истории вкладки «План»
```
