...
- GPT-план из трёх фаз и явного подневного календаря: исходная метрика, расчёты нагрузки, упражнения, подходы, точный объём, темп, отдых, критерий выполнения и правило корректировки;
...
[actum-ai] prompt=actum-plan-2026-08-01-daily-calendar-v4.1 research_prompt=actum-research-2026-08-01-v1.2 contract=plan-v4 validator=plan-validator-v4.1 baseline_parser=baseline-v1 transport=background-polling durable_state=ready
```
...
→ рассчитанные комплексы, подходы, таймеры работы/отдыха и критерии Actum
...
`plan-v4` не принимает общую фразу вместо тренировки. Пользователь отдельно вводит измеренную исходную точку — например, «обычно 40 секунд; сухой рекорд 1:20» — и свободный срок большой цели: месяц, год, два года и так далее. Локальный parser до платного планирования распознаёт явно подписанный рекорд как `80 seconds`; GPT не может подменить его собственным числом. Если среди нескольких значений нет однозначного максимума/рекорда, parser не угадывает baseline и процентные назначения запрещаются. Срок большой цели не ограничивает размером один OpenAI-ответ: Actum строит ближайший исполняемый блок на 7, 14 или 30 дней, а долгую цель сохраняет как направление.

Модель обязана вернуть ровно `horizonDays` самостоятельных объектов `days` с номерами `1…horizonDays`; `repeatCount`, «повтори вчера», скрытое клонирование и локальная перенумерация перепутанных дней больше не используются. В каждом дне полностью записаны действия, включая восстановление и отдых. Для комплекса модель возвращает фактический порядок, число подходов, объём (`повторы`, `секунды`, `минуты`, `страницы` и другие единицы), отдых, темп и критерий каждого действия. Runner превращает комплекс в последовательность `работа → отдых → следующий подход`; секунды и минуты отсчитываются автоматически. На экране «Путь» можно открыть детали любого дня выбранного блока, а check-in остаётся у текущего дня.

Если назначение рассчитано от baseline, GPT обязан вернуть строгий объект `{ percentage, baseValue, baseUnit, result }`, а приложение само показывает формулу `45% × 80 seconds = 36 seconds`. Сервер независимо сверяет исходное значение с локальным parser, пересчитывает процент, проверяет единицу и требует, чтобы `result` совпадал с фактическим `quantity`. Каждый action также содержит машинное `workSecondsPerSet`, поэтому длительность проверяется не только для секундных таймеров, но и для повторов, страниц и других дискретных действий. Динамическая JSON-схема заранее фиксирует точное число дней; известная длительность таймера или комплекса не может превышать ни дневной лимит, ни заявленное `estimatedMinutes`. Сервер больше не «исправляет» потенциально важную дозировку молчаливым сокращением работы или отдыха: несогласованный план отклоняется с точным путём поля, а завершённый OpenAI-этап остаётся в кэше и не вызывается повторно.

Правило корректировки показывается рядом с каждым днём. Check-in пока не переписывает будущие числа автоматически; результат и заметка сохраняются, а ветка уменьшения/повтора остаётся явной инструкцией. Автоматическое создание следующей версии календаря по накопленным метрикам — следующий слой после этого MVP.
...
`pnpm test:ai` использует фальшивый OpenAI, не открывает сетевые сокеты и не тратит API-деньги. Он воспроизводит сбои POST и polling, проверяет восстановление guard/response ID/кэшей, resume и terminal retry по тому же response ID, сохранение research, `inflight_join`, кэш готового плана и отдельно доказывает, что `plan-v4` возвращает ровно 30 явных дней, сохраняет baseline/targetTimeline, локально разбирает измерения, правильно считает проценты и отклоняет общие фразы, переставленные дни, разрывы фаз и противоречивую длительность. `pnpm check` запускает TypeScript, эти тесты и production export web-версии.
